...
<div class="overflow-x-auto px-6 py-12 section-reveal {className}">
  <table class="w-full border-collapse">
    <caption class="sr-only">{t?.('landing.FeatureComparison.caption') ?? 'Feature comparison'}</caption>
    <thead>
      <tr>
        <th scope="col" class="text-left py-4 px-4 text-label-md text-muted-foreground">{t?.('landing.FeatureComparison.feature') ?? 'Recurso'}</th>
        {#each plans as plan}
          <th scope="col" class="text-center py-4 px-4 text-label-md font-bold {plan.highlight ? 'text-primary' : 'text-foreground'}">
            {plan.name}
          </th>
        {/each}
      </tr>
