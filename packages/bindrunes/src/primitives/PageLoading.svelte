...
{:else if type === 'cards'}
  <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4" role="status" aria-label={t?.('common.loading') ?? 'Loading'}>
    {#each Array(rows) as _, r}
...
{:else if type === 'form'}
  <div class="space-y-4" role="status" aria-label={t?.('common.loading') ?? 'Loading'}>
