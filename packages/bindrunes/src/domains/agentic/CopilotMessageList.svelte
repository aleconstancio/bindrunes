...
let messagesContainer: HTMLDivElement = $state(null as unknown as HTMLDivElement);
...
{#if messages.length === 0 && !streamingContent && suggestions.length > 0}
	<div class="p-4 border-b border-border">
		<h3 class="text-xs font-bold uppercase tracking-widest text-muted-foreground mb-3">Sugestões</h3>
		<div class="space-y-2">
			...
			<div class="text-center max-w-md">
				...
				{#if mode === "global"}
					<div class="flex flex-wrap gap-2 justify-center">
						<button
							type="button"
							class="inline-flex items-center justify-center rounded-md border border-border bg-background hover:bg-muted px-3 py-1.5 text-sm font-medium"
			...
			{#each messages as message (message.id)}
				<div class="flex gap-3 {message.role === 'user' ? 'flex-row-reverse' : ''}">
					<div class="shrink-0 w-7 h-7 rounded-full flex items-center justify-center
						...
						{:else}
							<svg class="w-3.5 h-3.5" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>
						{/if}
					</div>
