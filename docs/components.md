...
## Boundrune — Composable Page Blocks

Pre-composed page sections for rapid application development. Import from `bindrunes/boundrune`. All blocks are themed (respect three-axis design system), data-driven via typed props, and composable — stack them to build pages.

### Application Blocks

| Block | What it wraps | When to use |
|-------|---------------|-------------|
| `<AdvancedTable>` | DataTable + search + Pagination + create button | Full data listing page |
| `<CrudCreateForm>` | Block wrapper + Form with header | Inline create form on a page |
| `<CrudCreateDrawer>` | Sheet + Form in footer | Create via side panel |
| `<CrudCreateModal>` | Dialog + Form | Create via modal overlay |
| `<CrudDetailDrawer>` | Sheet + Badge/Skeleton | Read-only detail in side panel |
| `<CrudDetailSection>` | Block + badge grid | Inline read-only detail on a page |
| `<CrudDeleteConfirm>` | Dialog + destructive Button | Confirm-then-delete flow |
| `<CrudEditForm>` | Block + Form with initial values | Edit existing resource |
| `<TableHeader>` | Sortable column headers + action slot | Table toolbar |
| `<FacetedSearch>` | Search input + filter selects | Data filtering bar |
| `<SettingsSection>` | Block + Card with form body | Settings page sub-section |
| `<DashboardFooter>` | Minimal footer with links | App-level footer |

### Marketing Blocks

| Block | Description |
|-------|-------------|
| `<Banner>` | Dismissible sticky banner (top/bottom) with CTA |
| `<Popup>` | Modal popup with badge, title, description, CTA |
| `<Portfolio>` | Image grid with hover zoom and tag overlays |
| `<BlogArticle>` | Full article with header, body, tags, author |
| `<ContentWithImage>` | Side-by-side text + image sections |
| `<CommentSection>` | Comment list + submit form |
| `<Schedule>` | Timeline event list with date markers |
| `<SocialProof>` | Testimonial cards + logo cloud |
| `<Maintenance>` | Full-page maintenance notice with gradient |

### Auth Blocks

| Block | Description |
|-------|-------------|
| `<LoginForm>` | Sign-in form with email/password, show/hide toggle, forgot-password link |
| `<RegisterForm>` | Registration form with name/email/password, validation |
| `<ForgotPassword>` | Email input + send reset link flow |
...
```svelte
<script lang="ts">
  import { AdvancedTable, CrudCreateDrawer, CrudDeleteConfirm } from "bindrunes/boundrune";
  import { createForm } from "bindrunes";
  import { email, minLength, object, string } from "valibot";

  const userForm = createForm({
    schema: object({ name: string(), email: string([email()]) }),
    onSubmit: async (data) => { /* POST */ },
  });
</script>

<AdvancedTable {columns} {rows} bind:searchValue onCreate={() => showDrawer = true} />

<CrudCreateDrawer bind:open={showDrawer} config={{ title: "New User", fields: [...] }} form={userForm}>
...
All components follow these a11y patterns:

- Interactive elements have `role` attributes where appropriate
- Icon-only buttons support `ariaLabel` prop
- Dialogs have `role="dialog"`, `aria-modal="true"`, and focus trapping (via bits-ui)
- Tabs use `role="tablist"`, `role="tab"`, `role="tabpanel"` (via bits-ui)
- Form inputs have associated labels via `aria-label` or `aria-labelledby`
