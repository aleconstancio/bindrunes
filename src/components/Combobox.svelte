...
<BitsCombobox.Root bind:value bind:search {disabled} class="relative {className}">
	<BitsCombobox.Input
		{placeholder}
	...
	<BitsCombobox.Portal>
		<BitsCombobox.Content class="z-[--z-overlay,30] mt-1 max-h-60 w-full overflow-auto rounded-[--radius] border bg-card p-1 shadow-md">
			<BitsCombobox.Viewport>
				{#each options.filter((o) => o.label.toLowerCase().includes(search.toLowerCase())) as option}
					<BitsCombobox.Item
						value={option.value}
