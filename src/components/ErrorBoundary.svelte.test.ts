...
describe('ErrorBoundary', () => {
	...
	it('showHome=false hides home button', async () => {
		...
		expect(container.textContent).not.toContain('Página Inicial');
	});

	it('showHome=true shows home button with custom url', async () => {
		const { container } = render(ErrorBoundaryHarness, { text: 'X', showHome: true, homeUrl: '/dashboard' });
		window.dispatchEvent(new ErrorEvent('error', { message: 'X' }));
		await vi.waitFor(() => {
			expect(container.textContent).toContain('Página Inicial');
		});
	...
	it('showRetry=false hides retry button', async () => {
		...
		expect(container.textContent).not.toContain('Recarregar');
	});
