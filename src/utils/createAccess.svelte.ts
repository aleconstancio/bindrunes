...
export function createAccess() {
  ...
  return {
    ...
    get permissions() { return auth.permissions; },
    get tenantId() { return auth.tenantId; },
    ...
    can(options: { roles?: string[]; permissions?: string[]; requireAll?: boolean }): boolean {
      ...
      const roleOk = !requiredRoles?.length || (requireAll
        ? auth.hasAllRequired(requiredRoles)
        : auth.hasAnyRole(requiredRoles));

      const permOk = !requiredPerms?.length || (requireAll
        ? auth.hasAllRequired(requiredPerms)
        : auth.hasAnyPermission(requiredPerms));

      return roleOk && permOk;
    },

    get isAdmin() { return auth.hasRole('admin'); },
    get isModerator() { return auth.hasRole('moderator'); },
  };
}
