...
export function createThemeBuilder(options: ThemeBuilderOptions) {
  ...
  const background = options.background ?? (isLight
    ? `oklch(0.98 0.01 ${hue})`
    : `oklch(0.05 0.01 ${hue})`
  );
  ...
  const primaryForeground = isLight
    ? `oklch(0.99 0 0)`
    : deriveFromPrimary(primary, -0.55, 0.1);

  const accentForeground = isLight
    ? `oklch(0.99 0 0)`
    : deriveFromPrimary(accent, -0.55, 0.1);
