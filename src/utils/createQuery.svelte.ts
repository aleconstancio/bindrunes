...
export function createQuery<TData>(options: CreateQueryOptions<TData>): QueryResult<TData> {
	...
	$effect(() => {
		...
		const unsubscribe = subscribe(options.key, () => {
			...
			if (prevStatus !== _status) {
				if (_status === "success") {
					if (_data !== undefined) options.onSuccess?.(_data);
	...
	async function refetch(): Promise<void> {
		...
		if (entry) {
			if (entry.status === "success" && entry.data !== undefined) {
				...
				if (entry.error) options.onError?.(entry.error);
			}
		}
	}
	...
	return {
		...
		get isStale() {
			return _lastUpdatedAt === 0 || Date.now() - _lastUpdatedAt > (options.staleTime ?? 0);
		},
		refetch,
	};
}
